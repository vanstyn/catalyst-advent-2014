...
don't want to fuss with ACCEPT_CONTEXT, passing it around in argument lists, and so on. 

Isn't there an easier way? There is! read on...
...
"localize" a reference to C<$c> to be globally accessible, but only within the appropriate scope.
...
happens during a normal request cycle is ultimately contained within the lexical scope of the 
C<dispatch()> method.
