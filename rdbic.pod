...
to bootstrap new applications which which work out-of-the-box.
...
single connect info (DSN) argument. 
...
required) appended with commas  as shown above(no spaces).
...
be supplied as simply the path to the database file:
...
To learn more about the extensive L<RapidApp> project ...
